...
def learn(nnmovies,ratings):
    nn = MLPRegressor(hidden_layer_sizes=200, activation="logistic", solver="adam", verbose=True, max_iter=3000, learning_rate="adaptive")
    x = []
    y = []
...
def prepareMoviesForNN(movies):
    ...
    roleBinarizer = binariseRoles(getAllRoles(movies))
    personEncoder = personIdEncoder(movies)
    for movie in movies:
        ...
        genre =  np.add(genre1,genre2,genre3)
        actors = []
        #for i in range(10):
        #    try:
...
def predictMovie(startYear, runtime, genre1, genre2, genre3):
    genresBinarizer = binariseGenres()
    genre1 = np.array(genresBinarizer.transform([genre1])[0])
    genre2 = np.array(genresBinarizer.transform([genre2])[0])
    genre3= np.array(genresBinarizer.transform([genre3])[0])
    genre = np.add(genre1, genre2, genre3)
    mlmovie = np.array([startYear, runtime])
    mlmovie = np.concatenate((mlmovie,genre))
    mlmovie = mlmovie.reshape(1, -1)
    return mlmovie


if __name__ == '__main__':
    moviePredictor = learn(prepareMoviesForNN(loadDataBase()),getYValues(loadDataBase()))
    print(moviePredictor.predict(predictMovie(2002,90,"Romance", "Comedy","Thriller")))
